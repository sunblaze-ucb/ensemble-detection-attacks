...
# x_star_avg = tf.nn.avg_pool(x_star, [1, 2, 2, 1], [1, 1, 1, 1], padding='SAME', data_format='NHWC')
x_star_med = median.median_filter(x_star, 2, 2)
x_star_med = tf.reshape(x_star_med, (N0, H0, W0, C0))
x_star_scaled = tf.map_fn(lambda img: tf.image.per_image_standardization(img), x_star_med)
...
for _ in range(9):
    ...
    np.save(eval_dir + '/smoothing_adv_imgs.npy', best_images)
    np.save(eval_dir + '/smoothing_adv_coeff_log.npy', best_coeff_log)
    ...
    for i, (fail, curr, high, low) in enumerate(zip(all_fail, coeff_curr_log, coeff_high_log, coeff_low_log)):
        if fail:
            ...
            if math.isnan(low):
                ...
                coeff_curr_log[i] = (high + low) / 2
    np.save(eval_dir + '/smoothing_coeff_log.npy', coeff_block_log)
