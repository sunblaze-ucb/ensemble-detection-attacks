...
# Other branch for judging whether each image works as adversarial example
...
predictions = tf.argmax(model_quantized.logits, 1)
...
for _ in range(9):
    ...
    np.save(eval_dir + '/baseline2_adv_imgs.npy', best_images)
    np.save(eval_dir + '/baseline2_adv_coeff_log.npy', best_coeff_log)
    ...
    for i, (fail, curr, high, low) in enumerate(zip(all_fail, coeff_curr_log, coeff_high_log, coeff_low_log)):
        if fail:
            ...
            if math.isnan(low):
                ...
                coeff_curr_log[i] = (high + low) / 2
    np.save(eval_dir + '/baseline2_coeff_log.npy', coeff_block_log)
