...
from lib.median import median_filter
...
# Cmd Line Example: python median.py 3 3 100 5000 0

arg_names = ['med_filt_height', 'med_filt_width', 'n_adv_examples', 'n_adv_iters', 'restore_from_checkpoint']
...
adv_lr = 2e-2
results_dir = 'results/median{}x{}/'.format(med_filt_height, med_filt_width)
print_every = 500

######################
# Data Configuration #
######################
chkpt_dir = 'weights/ckpts_median{}x{}/'.format(med_filt_height, med_filt_width)
data_dir = 'mnist_data'
...
X_pp = median_filter(X, med_filt_height, med_filt_width)
X_pp = tf.reshape(X_pp, (-1, INPUT_DIM, INPUT_DIM, INPUT_CHANNELS))
...
X_pp = median_filter(x_star, med_filt_height, med_filt_width)
X_pp = tf.reshape(X_pp, (-1, INPUT_DIM, INPUT_DIM, INPUT_CHANNELS))
...
adv_dict = { X: x, y: ground_truth, tr_mode: False }

best_adv_imgs = np.copy(x)
best_l2       = -1.0 * np.ones(n_adv_examples)

for j in range(n_adv_iters):
    sess.run(train_adv_step, feed_dict=adv_dict)
    # TODO: should this collection be performed after each iteration? Might make it too slow...
    batch_l2, batch_acc, batch_xs = sess.run([l2dist, correct_pred, x_star], feed_dict=adv_dict)
...
avg_l2 = np.mean(np.sqrt(best_l2[best_l2 >= 0]))
np.save(results_dir + 'adversarial_images.npy', best_adv_imgs)
print(fstr.format(n_adv_iters, n_adv_iters, model_acc, avg_l2))
